...
export async function initSchema() {
  ...
  if (existing.rowCount === 0) {
    // 1. Crear admin y maitre/mesero básicos
    await pool.query(
    ...
    // 2. Crear 21 cocineros con especialidades
    for (let i = 1; i <= 5; i++) {
      await pool.query(
    ...
    // 3. Crear Mesas (Total 50 personas)
    ...
    // 4. Crear Menú Colombiano (47 ítems)
    const menuItems = [
